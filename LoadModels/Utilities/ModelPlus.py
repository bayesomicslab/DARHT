...
class Ensemble_Efficient():
    ...
    def validateD(self, dataLoader, adv = False, clean = False):
        ...
        for j in range(0, len(pred)):
            ...
            if adv and torch.sum(pred[j] == m) > 1:
                if not clean:
                    acc += 1
            elif pred[j].argmax(axis=0) == target[j]:
                acc += 1
...
class ModelPlus():
    ...
    def __init__(self, modelName, model, device, imgSizeH, imgSizeW, batchSize, n_classes = 10, normalize = None, jelly = False, TiT = False):
