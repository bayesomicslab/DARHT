...
class DefenseBarrageNetwork():
    ...
    def Group3Swirl(self, img):
        ...
        rMin = int(np.round(imgRows*(10.0/256.0)))
    ...
    def Group5FFTPerturbation(self, img):
        ...
        if self.ColorChannelNum == 3:
            img2 = np.copy(img)
            for i in range(3):
                im_fft = scipy.fftpack.fft2(img[:,:,i])
                ...
                im_new = scipy.fftpack.ifft2(im_fft).real
            ...
            im_fft = scipy.fftpack.fft2(img[:,:,0])
            # Set r and c to be the number of rows and columns of the array.
            r, c = im_fft.shape
    ...
    def Group6p1ZoomRandom(self, img):
        ...
        minValue = int(np.round(h*(10.0/256.0)))
    ...
    def Group9p1GaussianBlur(self, img):
        ...
        if self.ColorChannelNum == 3: #RGB image
            if self.randUnifC(0, 1) > 0.5:
                sigma = [self.randUnifC(0.1, 3)]*3 #Blur each channel with the same parameters 
            else:
                sigma = [self.randUnifC(0.1, 3), self.randUnifC(0.1, 3), self.randUnifC(0.1, 3)] #Blur each channel with a different parameter
        ...
        if self.ColorChannelNum == 1: #Grayscale image 
            sigma = [self.randUnifC(0.1, 3)]*3 #3 is extra here, don't actually need it
            #img[:,:,0] = skimage.filters.gaussian(img[:,:,0], sigma=sigma[0])
            img2[:,:,0] = skimage.filters.gaussian(img[:,:,0], sigma=sigma[0])
        return img2 
    ...
    def Group9p2MedianFilter(self, img):
        if self.ColorChannelNum == 3: #RGB image 
            if self.randUnifC(0, 1) > 0.5:
                radius = [self.randUnifI(2, 5)]*3
            else:
                radius = [self.randUnifI(2, 5), self.randUnifI(2, 5), self.randUnifI(2, 5)]
